...
| [menubar-spacer](https://github.com/nlink-jp/menubar-spacer) | Swift | Adjusts the spacing between macOS menu bar icons and puts it back exactly as it was — four presets drawn in the window at the widths measured on macOS 27, so they can be compared before anything is written; the state the Mac held beforehand is recorded before the first write and restored on demand. No permissions of any kind. darwin/arm64 |
...
| [net-meter](https://github.com/nlink-jp/net-meter) | Swift | macOS menu-bar meter for one network interface — upstream/downstream rate as numbers and a scrolling graph, with a panel for history, addresses, peaks and totals. No permissions, nothing else. macOS 26+, darwin/arm64 |
